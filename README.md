...
- Last updated: May 19, 2026 (DEMS overlay extended to the 4 Featured Girls cards; see "Recent Changes")
...
- The **4 Featured Girls** carousel cards use the same DEMS overlay (bottom-right, blurred backdrop) so the featured row matches the in-grid creator cards. Rendered by `FeaturedCarousel` in [`app/web-vite/src/components/AdvertisingSpaces.tsx`](app/web-vite/src/components/AdvertisingSpaces.tsx); `escort_type` is returned by `GET /creators/by-names` (already part of `CARD_COLUMNS`, no API change needed) (2026-05-19).
