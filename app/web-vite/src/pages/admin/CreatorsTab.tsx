...
export function CreatorsTab({
  ...
  // Single combined list. Sort order:
  //   1) verified first  2) active before deactivated
  //   3) "by invite" needs-attention first (none/failed -> read)
  //   4) most real images first.
  const inviteRank: Record<string, number> = { failed: 1, queued: 2, sent: 3, delivered: 4, read: 5 };
  const inviteRankOf = (id: string) => {
    const st = inviteStatuses[id];
    return st ? (inviteRank[st.status] ?? 0) : 0;
  };
  const visible = creators.filter(matches).sort((a, b) => {
    if (a.verified !== b.verified) return a.verified ? -1 : 1;
    if (a.is_active !== b.is_active) return a.is_active ? -1 : 1;
    const ra = inviteRankOf(a.id), rb = inviteRankOf(b.id);
    ...
    <>
      ...
      <div className="rounded-3xl border border-brand-line bg-brand-surface/55 p-7 shadow-luxe">
        <div className="flex items-center justify-between gap-3">
          <div className="text-xs tracking-[0.22em] text-brand-muted">CREATORS ({visible.length})</div>
          <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-brand-line text-left text-xs tracking-[0.18em] text-brand-muted">
                ...
                <th className="pb-3 pr-4 font-normal">IMAGES</th>
                <th className="pb-3 pr-4 font-normal">VERIFIED</th>
                <th className="pb-3 pr-4 font-normal">ACTIVE</th>
                <th className="pb-3 pr-4 font-normal">BY INVITE</th>
                <th className="pb-3 font-normal">ACTIONS</th>
              </tr>
            </thead>
            <tbody>
              {visible.length === 0 ? (
                <tr><td colSpan={10} className="py-4 text-xs text-brand-muted">{search ? "No matches." : "No creators."}</td></tr>
              ) : visible.map((c, i) => (
                <tr key={c.id} className="border-b border-brand-line/40 last:border-0">
                  <td className="py-3 pr-4 text-xs text-brand-muted">{i + 1}</td>
                  ...
                  <td className="py-3 pr-4 text-xs text-brand-muted">{c.image_count}</td>
                  <td className="py-3 pr-4">
                    <button
                      type="button"
                  ...
                  <td className="py-3 pr-4">
                    <span
                      title={c.is_active ? "Active" : "Deactivated"}
                      aria-label={c.is_active ? "Active" : "Deactivated"}
                      className={`inline-block h-3 w-3 rounded-full ${c.is_active ? "bg-emerald-500" : "bg-red-500"}`}
                    />
                  </td>
